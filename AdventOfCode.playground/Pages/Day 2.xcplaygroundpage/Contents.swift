...
struct PasswordRule {
    let initialCharacterIndex: Int
    let secondCharacterIndex: Int
    let requiredCharacter: String

    init?(ruleString: String) {
        let components = ruleString.components(separatedBy: " ")
        guard components.count == 2 else { return nil }
        requiredCharacter = components[1]

        let ruleComponents = components[0].components(separatedBy: "-")
        initialCharacterIndex = Int(ruleComponents[0])!
        secondCharacterIndex = Int(ruleComponents[1])!
    }

    func isPasswordValid(password: String) -> Bool {
        guard password.count > secondCharacterIndex else { return false }

        let initialCharacter = String(password[initialCharacterIndex])
        let secondCharacter = String(password[secondCharacterIndex])

        var matches = 0
        if initialCharacter == requiredCharacter { matches = matches + 1 }
        if secondCharacter == requiredCharacter { matches = matches + 1 }
...
let validPasswords = inputs.lazy.filter { input -> Bool in
    ...
    guard let rule = PasswordRule(ruleString: String(ruleString)) else { return false }
    return rule.isPasswordValid(password: password)
}
